...
public class RtsCam extends AbstractAppState {
    ...
    private int[] direction2 = new int[5];
    ...
    public void initialize(AppStateManager stateManager, Application app) {
        ...
        displaysize = new Vector2f(app.getContext().getSettings().getWidth(), app.getContext().getSettings().getHeight());
    }
    ...
    public void update(final float tpf) {
        direction2 = new int[5];
        Vector2f pos = inputManager.getCursorPosition();
        if (pos.x < 30) {
            direction2[SIDE] = -1;
        } else if (pos.x > displaysize.x - 30) {
            direction2[SIDE] = 1;
        }
        if (pos.y < 30) {
            direction2[FWD] = 1;
        } else if (pos.y > displaysize.y - 30) {
            direction2[FWD] = -1;
        }
        for (int i = 0; i < direction.length; i++) {
            int dir = direction[i] | direction2[i];
